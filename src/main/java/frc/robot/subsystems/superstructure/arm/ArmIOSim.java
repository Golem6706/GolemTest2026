...
public class ArmIOSim implements ArmIO {
    ...
    private Voltage requsetedVoltage;
    ...
    public ArmIOSim() {
        ...
        this.requsetedVoltage = Volts.zero();
    ...
    public void updateInputs(ArmInputs armInputs) {
        ...
        AngularVelocity motorAngularVelocity = RadiansPerSecond.of(armSim.getVelocityRadPerSec() * HARDWARE_CONSTANTS.ARM_GEARING_REDUCTION());
        Voltage actualOuVoltage = simMotorController.constrainOutputVoltage(motorAngle, motorAngularVelocity, requsetedVoltage);
        actualOuVoltage = SimulatedBattery.clamp(actualOuVoltage);
        if(DriverStation.isDisabled())
            actualOuVoltage = Volts.zero();
        armSim.setInputVoltage(actualOuVoltage.in(Volts));

        //Run 10 iterations of the physics simulation to improve accuracy
        for(int i = 0; i < 10; i++)
            armSim.update(Robot.defaultPeriodSecs / 10);

        armInputs.absoluteEncoderAngle = Optional.of(Rotation2d.fromRadians(armSim.getAngleRads()));
        armInputs.motorConnected = true;
        armInputs.relativeEncoderAngledRad = motorAngle.plus(relativeEncoderOffset).in(Radians);
        ...
        armInputs.motorOutputVolts = actualOuVoltage.in(Volts);
    }
