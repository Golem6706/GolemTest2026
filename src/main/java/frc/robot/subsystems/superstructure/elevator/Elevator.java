...
public class Elevator extends SubsystemBase {
    ...
    private static final double CHAIN_LENGTH_METERS = HARDWARE_CONSTANTS.CHAIN_LENGTH().in(Meters);
    
    public Elevator (ElevatorIO io) {
        this.io = io;
        this.inputs = new ElevatorInputsAutoLogged();
        this.feedforwardController = new ElevatorFeedforward(PID_CONSTANTS.kS(), PID_CONSTANTS.kG(), PID_CONSTANTS.kV(),PID_CONSTANTS.kA());
     ...
     * <p>The elevator will slide down due to gravaity
     * <p>Caution: The elevator will hit the basement really hard if brake mode is off.
     */
    ...
    private void executeControlLoops(double dtSeconds) {
        dtSeconds = MathUtil.clamp(dtSeconds, 0, 0.1);  //clamp the value of tSeconds between 0 and 0.1
        ...
        double outputVolts = MathUtil.clamp(feedforwardVolts + feedbackVolts, 
            PID_CONSTANTS.MIN_OUTPUT_VOLTAGE().in(Volts), 
            PID_CONSTANTS.MAX_OUTPUT_VOLTAGE().in(Volts));

        //Get the setpoin    
        if (goalState.position == 0.0 && currentStateMeters.position == 0.0 && atReference())
            outputVolts = 0;
    ...
    /**
     * Wether the elevator's profile state is close enough to its setpoint.
     * <p>Note that this does not reflect whether the mechanism is actually at its setpoint
     * @return<code> true </code> If there is a setpoint and the profile state is close enough to it, <code> false </code> otherwise.
     */
    ...
    public void periodic() {
        ...
        if (DriverStation.isDisabled())         // Disable PID setpoint if the robot is disabled
            executeIdle();
        ...
        if (getHeightMeters() < PID_CONSTANTS.TOLERANCE().in(Meters)) {
            ...
            > HARDWARE_CONSTANTS.ELEVATOR_MAX_HEIGHT().in(Meters) + PID_CONSTANTS.TOLERANCE().in(Meters)) {
        ...
        // Tell drivetrain to lower speed if lov speed mode enable
        ...
        Logger.recordOutput("Elevator/Current State  Postion (Meters)", currentStateMeters.position);
     ...
     * <p><b> Note: This command finishes when the elevator reaches the setpoint, causing the default command to schedule. </b>
     * @param elevatorHeightSetpoint
     */
     ...
     * <p><b>CAUTION: It is dangerous to disable motor brake when the robot is enable. Since the elevator will hit the 
     * base hardly when the motor is in idle output. </b>
     */
