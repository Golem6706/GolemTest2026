...
public class SuperStructureVisualizer {
    ...
    public static void visualizeMechanisms(String key, double elevatorHeightMeters, Rotation2d armAngle) {
        ...
        Rotation3d armRotaion = new Rotation3d(0, ARM_ZERO_PITCH_RAD - armAngle.getRadians(), 0);
        Pose3d[] poses = new Pose3d[] {
            ...
            new Pose3d(ARM_ZERO_POSITION.plus(ELEVATOR_ZERO_POSITION).plus(elevatorHeightTranslation), armRotaion)
        };
    ...
    public static Pose3d getCoralOnRobotPosition(
        ...
        Pose3d coralPoseOnRobot = getCoralPosiontOnRobotRelative(elevatorHeightMeters, armAngle, coralDisplacement);
        Rotation3d coralOrientation = coralPoseOnRobot.getRotation()
                .plus(new Rotation3d(0.0,0.0, robotPose.getRotation().getRadians()));
        
        Translation3d robotPosition = new Translation3d(robotPose.getTranslation());
        Translation3d coralPosition = robotPosition
                .plus(coralPoseOnRobot.getTranslation().rotateBy(new Rotation3d(robotPose.getRotation())));
        return new Pose3d(coralPosition, coralOrientation);
    }
   
    public static Pose3d getCoralPosiontOnRobotRelative(
                double elevatorHeightMeters, Rotation2d armAngle, Distance coralDisplacement) {
        // Coral Orientation on Robot,namely Pitch
        Rotation3d coralOrientationOnRobot = new Rotation3d(
                0, -armAngle.plus(ARM_ANGLE_TO_CORAL_POSITION_ANGLE).getRadians(), 0);
        // Get coral position (x, y, z) on robot      
        Translation3d coralPositionOnRobot = new Translation3d(0.27, 0, 0.365)
                .plus(new Translation3d(CORAL_LENGHT_ON_ARM.in(Meters),
                        new Rotation3d(0, -armAngle.plus(ARM_PINPOINT_TO_CORAL_DIRECTION).getRadians(),0)))
                .plus(new Translation3d(0.0, 0.0, elevatorHeightMeters).plus(ELEVATOR_ZERO_POSITION))
