...
public class CoralHolderIOReal implements CoralHolderIO{
    ...
    public CoralHolderIOReal() {
        ...
        rollerTalon.getConfigurator().apply(
                        ...
                        .withInverted(HARDWARE_CONSTANTS.rollerMotorInverted()
        ...
        this.feedersOutput = Arrays.stream(feederTalons).map(TalonFX::getSupplyVoltage).toArray(StatusSignal[]::new);
        ...
        for(int i = 0; i < feederTalons.length; i++) {    //Initialize the feederTalons
            feederTalons[i].getConfigurator()
                                .apply(new MotorOutputConfigs()
                                        .withInverted(HARDWARE_CONSTANTS.feederMotorsInverted()[i]
                                        ? InvertedValue.Clockwise_Positive
                                        : InvertedValue.CounterClockwise_Positive));
            feederTalons[i].getConfigurator()
        ...
        //Initialize the LaserCANs
        this.firstSensor = new LaserCan(HARDWARE_CONSTANTS.firstSeneorID());
    ...
    public void updateInputs(CoralHolderInputs inputs) {
        inputs.motorConnected = BaseStatusSignal.refreshAll(rollerMotorCurrent, rollerMotorOutputVoltage).isOK();
        if (feederTalons.length > 0) {//There is a bug in the following two lines code of motorConnected
        ...
        for (int i = 0; i < feederTalons.length; i++) {
            inputs.feederMotorOutputVolts += feedersOutput[i].getValueAsDouble();
            inputs.feederMotorCurrentAmps += feedersCurrent[i].getValueAsDouble();
        }
        inputs.feederMotorOutputVolts /= feederTalons.length;

        LaserCanInterface.Measurement firstSensoMeasurement = firstSensor.getMeasurement();
        if (firstSensoMeasurement == null || firstSensorConfigurationError) {
            ...
            inputs.firstSensorReadingValid = 
                    firstSensoMeasurement.status == LaserCan.LASERCAN_STATUS_VALID_MEASUREMENT;
            inputs.firstSensorDistanceMM = firstSensoMeasurement.distance_mm;
        }
